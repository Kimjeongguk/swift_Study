...
class LoginViewController: UIViewController {
    ...
    @IBAction func appleLoginButtonTapped(_ sender: Any) {
        self.startSignInWithApplleFlow()
    }
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            guard let nonce = currentNonce else {
                fatalError("Invalid stare: A login callback was received, but no login request was sent.")
            }
...
extension LoginViewController {
    func startSignInWithApplleFlow() {
        ...
        while remainingLength > 0 {
            let randoms: [UInt8] = (0..<16).map { _ in
                ...
                if errorCode != errSecSuccess {
                    fatalError("Unable to generate nonce. SecRandomCopyBytes failed with OSSatus \(errorCode)")
                }
                return random
            }
